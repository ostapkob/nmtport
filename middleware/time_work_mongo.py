...
def save_to_xlsx(avg_by_terminals):
    ...
    titles = ["date", "shift", "terminal", "number", "time", 'degrees90', 'degrees180', 'fio', 'contract', 'start', 'finish']
    for en, title in enumerate(titles, 1):
        ws.cell(row=1, column=en, value=title)
    for en, item in enumerate(avg_by_terminals, 2):
        date, shift, terminal, number, time, degrees90, degrees180, fio, contract, start, finish = item
...
if __name__ == "__main__":
    ...
    # date_start  = datetime(2021, 3, 23).date()
    date_finish = datetime(2022, 3, 23).date()
